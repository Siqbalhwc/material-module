...
type MaterialStockMovement = {
  product_id: string;
  code: string;
  displayName: string;         // hierarchical name
  category: string;
  uom: string;
...
type SortField =
  | "code" | "displayName" | "category" | "uom" | "reorder_level"
...
export default function MaterialStorePage() {
    ...
    for (const row of allProducts) {
      if (!uniqueMap.has(row.product_id)) {
        ...
        const parentName = isChild
          ? parentNameMap.get(prod.parent_product_id) ?? ""
          : "";
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(i =>
        i.displayName.toLowerCase().includes(q) || i.code.toLowerCase().includes(q)
      );
    }
    list.sort((a, b) => {
  ...
  const handleIssueToWIP = async () => {
    ...
    try {
      await supabase.from("store_transfers").insert({
        ...
        uom: issueItem.uom,
        status: "pending",
        notes: `Issue to WIP – ${issueItem.displayName}`,
      });
                    ...
                    {Object.entries(visibleColumns).map(([key, value]) => (
                      <label key={key} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                        ...
                        <span className="capitalize text-gray-600">
                          {key === "reorder_level" ? "Reorder Lvl" : key.replace(/_kg$/, " (KG)").replace(/_bags$/, " (Bags)").replace(/_/g, " ")}
                        </span>
                      </label>
                    ))}
                  </div>
        ...
        {issueItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 print:hidden">
            <div className="bg-white rounded-xl p-6 w-96 space-y-4">
              <h2 className="text-lg font-semibold">Issue to WIP: {issueItem.displayName}</h2>
