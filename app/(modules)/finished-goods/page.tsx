...
export default function FinishedGoodsPage() {
  ...
  // Issue to Dispatch modal
  const [issueItem, setIssueItem] = useState<FGStockMovement | null>(null);
  ...
  const fetchMovements = async () => {
    ...
    // Get all products ever in finished_goods
  ...
  // ── Fetch incoming transfers (to finished_goods, pending) ──
  ...
  const handleIssueToDispatch = async () => {
    if (!issueItem) return;
    ...
    try {
      await supabase.from("store_transfers").insert({
        from_store: "finished_goods",
        to_store: "dispatch",
        product_id: issueItem.product_id,
        quantity: qtyKg,
        uom: issueItem.uom,
        status: "pending",
        notes: `Issue to Dispatch – ${issueItem.name}`,
      });
      alert("Transfer sent to Dispatch.");
      fetchMovements();
      setIssueItem(null);
      setIssueQtyKg("");
      setIssueQtyBags("");
                    ...
                    return (
                      <tr key={item.product_id} className="hover:bg-gray-50">
                        ...
                        <td className="table-td print:hidden">
                          ...
                          >
                            <Send className="h-3 w-3 inline" /> Issue to Dispatch
                          </button>
                        </td>
                      </tr>
                    );
        ...
        {issueItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 print:hidden">
            <div className="bg-white rounded-xl p-6 w-96 space-y-4">
              <h2 className="text-lg font-semibold">Issue to Dispatch: {issueItem.name}</h2>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button className="btn-primary" disabled={issuing} onClick={handleIssueToDispatch}>
