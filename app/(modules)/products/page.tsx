...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({
    name: "", category: "", uom: "kg", is_rc: false, reorder_level: "0", conversion_kg: "", parent_product_id: "",
  });

  const [visibleColumns, setVisibleColumns] = useState({
    code: true, name: true, category: true, uom: true, kgPerBag: true, reorder: false, rc: false, status: true,
  });
  ...
  const [editForm, setEditForm] = useState({
    name: "", category: "", uom: "kg", is_rc: false, reorder_level: "0", conversion_kg: "", is_active: true,
  });
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      supabase.from("user_roles").select("role").eq("user_id", user.id).eq("role", "super_admin").maybeSingle().then(({ data }) => setIsSuperAdmin(!!data));
    });
  }, []);

  const fetchProducts = async () => {
    const { data, error } = await supabase.from("products").select("*").order("created_at", { ascending: false });
    if (error) { console.error("Failed to fetch products:", error); return; }
    ...
    for (const p of allProducts) {
      if (p.parent_product_id) children.push(p);
      else parentMap.set(p.id, { ...p, children: [] });
    }
  ...
  const parentOptions = useMemo(() => {
    ...
    return products.filter(p => p.category === form.category && !p.parent_product_id);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: any = { name: form.name, category: form.category, uom: form.uom, is_rc: form.is_rc, reorder_level: parseFloat(form.reorder_level) };
      if (form.uom === "bags" && form.conversion_kg) payload.conversion_kg = parseFloat(form.conversion_kg);
      if (form.parent_product_id) payload.parent_product_id = form.parent_product_id;
      const { data, error } = await supabase.from("products").insert([payload]).select().single();
      if (error) throw error;
      setProducts((prev) => [data as ProductWithParent, ...prev]);
      setShowForm(false);
      setForm({ name: "", category: "", uom: "kg", is_rc: false, reorder_level: "0", conversion_kg: "", parent_product_id: "" });
    } catch (err: any) { alert("Failed to create product: " + (err.message || "Unknown error")); }
    finally { setSaving(false); }
  };

  const toggleColumn = (key: keyof typeof visibleColumns) => setVisibleColumns((prev) => ({ ...prev, [key]: !prev[key] }));
  const toggleExpand = (productId: string) => setExpandedParents(prev => { const next = new Set(prev); if (next.has(productId)) next.delete(productId); else next.add(productId); return next; });

  const startEdit = (p: ProductWithParent) => {
    setEditingId(p.id);
    setEditForm({ name: p.name, category: p.category, uom: p.uom, is_rc: p.is_rc, reorder_level: String(p.reorder_level), conversion_kg: p.conversion_kg ? String(p.conversion_kg) : "", is_active: p.is_active });
  };
  const cancelEdit = () => setEditingId(null);
  const saveEdit = async (id: string) => {
    setSaving(true);
    try {
      const payload: any = { name: editForm.name, category: editForm.category, uom: editForm.uom, is_rc: editForm.is_rc, reorder_level: parseFloat(editForm.reorder_level), is_active: editForm.is_active };
      if (editForm.conversion_kg) payload.conversion_kg = parseFloat(editForm.conversion_kg);
      else payload.conversion_kg = null;
  ...
  const handleExport = () => {
    const flatData = products.map(p => ({ Code: p.code, Name: p.name, Category: p.category, UOM: p.uom, "Kg/Bag": p.conversion_kg || "", "Reorder Level": p.reorder_level, "Is RC": p.is_rc ? "TRUE" : "FALSE", Active: p.is_active ? "TRUE" : "FALSE", "Parent Product Name": p.parent_product_id ? products.find(x => x.id === p.parent_product_id)?.name || "" : "" }));
  ...
  const handleDownloadTemplate = () => {
    const template = [{ Name: "Example Product", Category: "Raw Material", UOM: "kg", "Kg/Bag": "", "Reorder Level": "0", "Is RC": "FALSE", "Parent Product Name": "" }];
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          let parentId = null;
          if (parentName) { const { data: parent } = await supabase.from("products").select("id").eq("name", parentName).maybeSingle(); if (parent) parentId = parent.id; }
          const payload: any = { name, category, uom, is_rc, reorder_level };
  ...
  const handleNuke = async () => {
    ...
    try {
      const tables = ["dispatch_items","dispatch_orders","wip_material_consumption","wip_batches","requisition_items","requisitions","fg_transfers","rc_movements","ogp_line_items","outward_gate_passes","igp_line_items","inward_gate_passes","production_runs","store_transfers","stock_ledger"];
    ...
    <div className="p-6">
      <PageHeader
        ...
        actions={
          <>
            {isSuperAdmin && (
              <>
                <button onClick={handleDownloadTemplate} className="btn-secondary text-xs"><Download className="h-3.5 w-3.5" /> Template</button>
                <button onClick={() => fileInputRef.current?.click()} disabled={importing} className="btn-secondary text-xs"><Upload className="h-3.5 w-3.5" /> {importing ? "Importing…" : "Import"}</button>
                <button onClick={handleExport} className="btn-secondary text-xs"><Download className="h-3.5 w-3.5" /> Export</button>
                <button onClick={handleNuke} className="btn-secondary text-xs text-red-600"><Trash2 className="h-3.5 w-3.5" /> Nuke</button>
                <input type="file" ref={fileInputRef} onChange={handleImport} accept=".xlsx,.xls,.csv" hidden />
              </>
            )}
            <button className="btn-primary" onClick={() => setShowForm((v) => !v)}><Plus className="h-4 w-4" /> Add Product</button>
          </>
        }
      />

      {importMsg && <div className="bg-blue-50 text-blue-700 p-3 rounded-lg text-sm mb-4">{importMsg}</div>}
        ...
        <div className="card p-6 max-w-2xl mb-6">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <select className="input" required value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value, parent_product_id: "" }))}>
            ...
            {form.category && parentOptions.length > 0 && (
              <div className="col-span-2">
                ...
                <select className="input" value={form.parent_product_id} onChange={(e) => setForm((p) => ({ ...p, parent_product_id: e.target.value }))}>
                  <option value="">-- None (standalone) --</option>
                  {parentOptions.map((p) => (<option key={p.id} value={p.id}>{p.name} ({p.code})</option>))}
                </select>
              </div>
            )}
            <div><label className="label">Name *</label><input className="input" required placeholder="Product name" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} /></div>
            <div>
            ...
            <div><label className="label">Reorder Level</label><input className="input" type="number" step="0.001" min="0" value={form.reorder_level} onChange={(e) => setForm((p) => ({ ...p, reorder_level: e.target.value }))} /></div>
              ...
              <div className="p-2 space-y-1">
                {Object.entries(visibleColumns).map(([key, value]) => (
                  <label key={key} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                    ...
                    <span className="capitalize text-gray-600">{key === "kgPerBag" ? "Kg/Bag" : key === "rc" ? "RC" : key}</span>
                  </label>
                ))}
              </div>
            </div>
      ...
      <div className="card overflow-hidden">
        {loading ? <div className="py-16 text-center text-gray-400">Loading…</div> :
          filteredProducts.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-gray-400"><ShoppingBag className="h-10 w-10 mb-3 opacity-30" /><p className="text-sm">{searchQuery ? "No products match your search" : "No products yet"}</p></div> :
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredProducts.map((p) => {
                ...
                if (isChild && p.parent_product_id && !expandedParents.has(p.parent_product_id)) return null;
