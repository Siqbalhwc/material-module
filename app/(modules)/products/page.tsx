...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({
    code: "", name: "", category: "", uom: "kg", is_rc: false, reorder_level: "0",
  });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await productsApi.create({ ...form, reorder_level: parseFloat(form.reorder_level) });
      setProducts((p) => [res.data?.data, ...p]);
      setShowForm(false);
      setForm({ code: "", name: "", category: "", uom: "kg", is_rc: false, reorder_level: "0" });
    } catch {}
          ...
          <div className="card p-6 max-w-2xl">
            ...
            <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input className="input" required placeholder="e.g. PPD-001"
                  value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} />
              </div>
              <div>
                <label className="label">Name *</label>
                <input className="input" required placeholder="Product name"
                  value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
              </div>
              <div>
                <label className="label">Category *</label>
                <input className="input" required placeholder="e.g. Raw Material"
              ...
              <div className="flex items-center gap-2 pt-5">
                <input type="checkbox" id="is_rc" className="rounded"
                  checked={form.is_rc} onChange={(e) => setForm((p) => ({ ...p, is_rc: e.target.checked }))} />
                <label htmlFor="is_rc" className="text-sm text-gray-700">Returnable Component (RC)</label>
              </div>
          ...
          {loading ? (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {["Code", "Name", "Category", "UOM", "Reorder Level", "RC", "Status"].map((h) => (
                  ...
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="table-td">
                      <span className={cn("badge",
                        p.is_active ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-100 text-gray-400")}>
