...
export default function OutwardGatePassDetailPage() {
  const { id } = useParams<{ id: string }>();
  const [gp, setGp] = useState<OutwardGatePassDetail | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setGp({
          id: header.id,
          ogp_number: header.ogp_number,
          customer_name: header.customers?.name ?? null,
    ...
    return (
      <div className="p-6">
        <main className="flex flex-col items-center justify-center py-16 text-gray-400 space-y-2">
          ...
          <Link href="/outward-gate-pass" className="text-sm text-brand-600 hover:underline">← Back to list</Link>
        </main>
      </div>
    );
  }
    ...
    <div className="p-6">
      <style jsx global>{`
        @media print {
          ...
          .print-only { position: absolute; left: 0; top: 0; width: 100%; padding: 20px 30px; }
      ...
      <div className="screen-only">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Outward Gate Pass: {gp.ogp_number}</h1>
            <p className="text-sm text-gray-500 mt-1">Dispatched on {formatDate(gp.dispatch_date)}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={handlePrint} className="btn-secondary inline-flex items-center gap-1">
              <Printer className="h-4 w-4" /> Print / PDF
            </button>
            <Link href="/outward-gate-pass" className="btn-secondary inline-flex items-center gap-1">
        ...
        <div className="card p-6 space-y-6 max-w-4xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><span className="text-xs text-gray-500 font-medium">Customer</span><p className="text-sm font-medium text-gray-900 mt-1">{gp.customer_name || "—"}</p></div>
            ...
            <div><span className="text-xs text-gray-500 font-medium">Dispatch Date</span><p className="text-sm font-medium text-gray-900 mt-1">{formatDate(gp.dispatch_date)}</p></div>
          </div>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Dispatched Goods</h3>
              ...
              <tbody className="divide-y divide-gray-100">
                {gp.items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-3 py-2 text-xs font-medium text-gray-700 text-right">{item.dispatched_qty.toFixed(2)}</td>
      ...
      <div className="print-only">
        <div className="flex items-center justify-between border-b pb-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Outward Gate Pass</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-lg font-bold text-gray-900">{gp.ogp_number}</p>
            <p className="text-xs text-gray-500">Date: {formatDate(gp.dispatch_date)}</p>
            <span className={cn("inline-block mt-1 text-xs px-2 py-0.5 rounded-full border", STATUS_STYLE[gp.status])}>{gp.status}</span>
          </div>
        </div>
        ...
        {gp.notes && (
          <div className="mb-6"><span className="text-xs text-gray-500 font-medium">Notes</span><p className="text-sm text-gray-700 mt-1">{gp.notes}</p></div>
        )}

        <h3 className="text-sm font-semibold text-gray-700 mb-2">Dispatched Goods</h3>
